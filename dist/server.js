...
app.put("/api/products/:id", async (req, res) => {
    const { id } = req.params;
    const { name, cookingType, unitKg, unitsPerColli, collisPerPallet, packagingType, productNumber, } = req.body;
    if (!name || !cookingType || unitKg == null) {
        return res
            .status(400)
            .json({ error: "name, cookingType und unitKg sind Pflichtfelder" });
...
app.get("/api/varieties/by-farmer/:farmerId", async (req, res) => {
    ...
    try {
        ...
        movements.forEach((m) => {
            if (m.varietyId)
                varietyIds.add(m.varietyId);
        });
        runs.forEach((r) => {
            if (r.varietyId)
                varietyIds.add(r.varietyId);
        });
...
app.post("/api/farmers", async (req, res) => {
    const { name, street, postalCode, city, country, ggn, loginEmail, loginPassword, isFlatRate = false, flatRateNote = null, } = req.body;
    if (!name)
...
app.get("/api/statements/:filename", (req, res) => {
    ...
    const filepath = path_1.default.join(STATEMENTS_DIR, filename);
...
app.get("/api/admin/manual-costs", async (req, res) => {
    try {
        ...
        const where = {};
        if (packPlantId)
            where.packPlantId = Number(packPlantId);
        if (costType)
            where.costType = costType;
        if (dateFrom || dateTo) {
            where.OR = [];
            if (dateFrom) {
                where.OR.push({ periodTo: { gte: new Date(dateFrom) } });
            }
            if (dateTo) {
                where.OR.push({ periodFrom: { lte: new Date(dateTo) } });
            }
        }
...
app.post("/api/admin/manual-costs", async (req, res) => {
    try {
        const { costType, description, periodFrom, periodTo, packPlantId, valueType, value, comment, } = req.body;
...
app.get("/api/admin/pnl", async (req, res) => {
    try {
        ...
        const periodStart = dateFrom ? new Date(dateFrom) : new Date(new Date().getFullYear(), 0, 1);
        ...
        if (packPlantId) {
            ...
            salesWhere.id = {
                in: saleIds.map((m) => m.customerSaleId).filter((id) => id !== null),
            };
        }
        if (productId)
            salesWhere.productId = Number(productId);
        if (customerId)
            salesWhere.customerId = Number(customerId);
        ...
        function getOverlapWeeks(costStart, costEnd, calcStart, calcEnd) {
        ...
        for (const cost of manualCosts) {
            ...
            if (cost.valueType === "ABSOLUTE") {
                ...
                const overlapWeeks = getOverlapWeeks(costPeriodStart, costPeriodEnd, periodStart, periodEnd);
                ...
                if (cost.packPlantId) {
                    ...
                    costSalesWhere.id = {
                        in: costSaleIds.map((m) => m.customerSaleId).filter((id) => id !== null),
                    };
                }
...
app.get("/api/accounting-documents/:filename", (req, res) => {
    ...
    const filepath = path_1.default.join(ACCOUNTING_DOCS_DIR, filename);
    if (!fs_1.default.existsSync(filepath)) {
        return res.status(404).json({ error: "PDF nicht gefunden" });
    }
    res.setHeader("Content-Type", "application/pdf");
...
app.post("/api/farmer-stock/direct-sale", async (req, res) => {
    try {
        ...
        fieldName, harvestDate, sortierGroesse, // Sortiergröße für Kartoffeln
        ...
        if (saleType === "EG") {
            try {
                await applyPackStationStockChange(PACKSTATION_ID, farmerIdNum, varietyIdNum, qtyKg, "RAW_IN_FROM_FARMER", "Verkauf Bauer -> EG (Rohware in Packstelle)", sortierGroesse || null, varietyQuality || null);
                console.log("PackStationStockChange erfolgreich durchgeführt");
            }
            catch (packStationErr) {
...
app.get("/api/organizer/deliveries", async (req, res) => {
    ...
    try {
        ...
        const result = movements.map((m) => ({
            ...
            varietyQuality: m.varietyQuality ?? null, // NEU: erfasste Qualität für Lieferschein
            quantityKg: Number(m.changeKg), // RAW_IN_FROM_FARMER ist positiv
            sortierGroesse: m.sortierGroesse ?? null,
        }));
        res.json(result);
    }
    catch (err) {
...
async function applyPackStationStockChange(packStationId, farmerId, varietyId, changeKg, reason, comment, sortierGroesse, varietyQuality) {
    const snaps = {
    ...
    await prisma.packStationStockMovement.create({
        data: {
            ...
            reason,
            comment: comment ?? null,
            sortierGroesse: sortierGroesse ?? null,
            varietyQuality: varietyQuality ?? null,
            ...snaps,
        },
    });
}
...
async function recalculateProductPriceValidToForGroup(customerId, productId) {
    ...
    for (let i = 0; i < prices.length; i++) {
        ...
        const needsUpdate = (currentValidTo === null && newValidTo !== null) ||
...
async function recalculateVarietyQualityPriceValidTo(quality) {
...
async function getVarietyQualityPriceForDate(quality, date) {
...
async function getFarmerStatement(farmerId, dateFrom, dateTo) {
    ...
    const isFlatRate = !!farmer.isFlatRate;
    const flatRateNote = farmer.flatRateNote ?? null;
    // Adresse formatieren
    let addressText;
    if (farmer.address) {
        const parts = [
...
async function calculatePackstationStatsForPeriod(farmerId, periodStart, periodEnd) {
    ...
    const result = Array.from(statsByVariety.values()).map((stat) => {
        // Waschverlust = gelief. Menge - Bestand - verpackt - Ausschuss - Inventur
        const processedBaseWithoutWash = stat.packedKg + stat.wasteKg + stat.inventoryZeroKg;
        const washingLoss = stat.deliveredKg -
            stat.currentKg -
            processedBaseWithoutWash;
    ...
    return result.sort((a, b) => a.varietyName.localeCompare(b.varietyName, "de", {
        sensitivity: "base",
    }));
}
// ▶ Rückwärtskompatibilität: Monatsbezogene Funktion
async function getFarmerMonthlyStatement(farmerId, year, month) {
...
function formatAddressSnapshot(addr) {
    if (!addr)
        return null;
    const parts = [addr.street, addr.postalCode, addr.city, addr.country].filter((p) => !!p && String(p).trim().length > 0);
    return parts.length ? parts.join(", ") : null;
}
function buildDocumentNumber(documentType) {
    const now = new Date();
...
async function getIssuerSnapshot() {
    ...
    // Fallback auf ENV
    return {
        issuerNameSnapshot: process.env.INVOICE_ISSUER_NAME ||
...
const ACCOUNTING_DOCS_DIR = path_1.default.join(process.cwd(), "documents");
...
async function generateAccountingDocumentPdf(doc) {
    return new Promise((resolve, reject) => {
        try {
            ...
            pdf.on("error", reject);
            const title = doc.documentType === "GUTSCHRIFT"
                ? "GUTSCHRIFT"
            ...
            if (doc.servicePeriodFrom && doc.servicePeriodTo) {
                pdf.moveDown(0.6);
                pdf.text(`Leistungszeitraum: ${formatDateDE(new Date(doc.servicePeriodFrom))} - ${formatDateDE(new Date(doc.servicePeriodTo))}`, 50, pdf.y);
            }
            pdf.moveDown(1.5);
            ...
            if (doc.issuerAddressSnapshot) {
                ...
                const addressParts = doc.issuerAddressSnapshot.split(",").map((p) => p.trim());
            ...
            if (doc.issuerRegistryNumberSnapshot) {
                pdf.text(`Firmenbuch: ${doc.issuerRegistryNumberSnapshot}${doc.issuerRegistryCourtSnapshot ? `, Gericht: ${doc.issuerRegistryCourtSnapshot}` : ""}`, leftX, pdf.y);
                pdf.moveDown(0.25);
            }
            ...
            if (doc.recipientAddressSnapshot) {
                // Adresse in separate Zeilen aufteilen, falls Kommas vorhanden
                const addressParts = doc.recipientAddressSnapshot.split(",").map((p) => p.trim());
                addressParts.forEach((part) => {
                    if (part) {
            ...
            if (doc.recipientRegistryNumberSnapshot) {
                pdf.text(`Firmenbuch: ${doc.recipientRegistryNumberSnapshot}${doc.recipientRegistryCourtSnapshot ? `, Gericht: ${doc.recipientRegistryCourtSnapshot}` : ""}`, rightX, pdf.y);
                pdf.moveDown(0.25);
            }
            ...
            const sortedLines = [...doc.lines].sort((a, b) => {
            ...
            sortedLines.forEach((line, idx) => {
                ...
                pdf.text(Number.isFinite(unitPriceNum) && !isNaN(unitPriceNum) ? formatCurrency(unitPriceNum) : "0,00 €", colUnitPrice, y, { width: 60, align: "right" });
                // Gesamtpreis netto
            ...
            pdf.text("Diese Rechnung wurde maschinell erstellt und ist ohne Unterschrift gültig.", { align: "center", width: 500 });
...
async function saveAccountingDocumentPdf(doc, pdfBuffer) {
    if (!fs_1.default.existsSync(ACCOUNTING_DOCS_DIR)) {
        fs_1.default.mkdirSync(ACCOUNTING_DOCS_DIR, { recursive: true });
    }
...
async function generateFarmerStatementPdf(statement) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (statement.isFlatRate) {
                ...
                doc.text(statement.flatRateNote ||
            ...
            doc.text(`USt (${statement.isFlatRate ? "13 % pauschal" : "0 %"})`, col2, y);
            ...
            doc.text("Diese Abrechnung ist eine Akonto-Zahlung. Die endgültige Jahresabrechnung erfolgt nach Abschluss des Wirtschaftsjahres unter Berücksichtigung des zur Verfügung stehenden Auszahlungsbetrages.", col1, y, { width: 500, align: "left" });
            y += 30;
            // === HINWEISTEXT ===
            doc.fontSize(8).font("Helvetica").fillColor("#666666");
            // Hinweis für pauschalierte Landwirte
            if (statement.isFlatRate) {
                doc.text("Bei pauschalierten Landwirten wird der Durchschnittssteuersatz von 13 % gemäß § 22 UStG angewendet.", col1, y, { width: 500, align: "left" });
                y += 30;
            }
            doc.text("Einsprüche gegen diese Abrechnung sind innerhalb von 14 Tagen schriftlich an die Erzeugergemeinschaft zu richten.", col1, y, { width: 500, align: "left" });
            y += 30;
            // === PACKSTELLEN-STATISTIK ===
            // Statistiken für den Abrechnungszeitraum berechnen
            const packstationStats = await calculatePackstationStatsForPeriod(statement.farmerId, statement.periodStart, statement.periodEnd);
...
async function sendFarmerStatementEmail(params) {
...
// ▶ PDF-Speicherung
const STATEMENTS_DIR = path_1.default.join(process.cwd(), "statements");
async function saveFarmerStatementPdf(statement, pdfBuffer) {
    // Ordner erstellen falls nicht vorhanden
    if (!fs_1.default.existsSync(STATEMENTS_DIR)) {
        fs_1.default.mkdirSync(STATEMENTS_DIR, { recursive: true });
    }
...
async function createPackPlantSalesInvoice(packPlantId, dateFrom, dateTo) {
    const periodStart = new Date(dateFrom);
    periodStart.setHours(0, 0, 0, 0);
    const periodEnd = new Date(dateTo);
    periodEnd.setHours(23, 59, 59, 999);
    const packPlant = await prisma.packPlant.findUnique({
    ...
    for (const sale of sales) {
        const vatRatePercent = Number(sale.product?.taxRate?.ratePercent ?? 0);
    ...
    for (const complaint of complaints) {
        ...
        if (complaint.complaintType === "RETOURWARE") {
            ...
            vatRatePercent = Number(sale?.product?.taxRate?.ratePercent ?? 0);
        }
        else {
        ...
        const complaintTypeText = complaint.complaintType === "RETOURWARE"
            ? "Retour/Reklamation"
...
async function createPackingCostCreditNote(packPlantId, dateFrom, dateTo) {
    const periodStart = new Date(dateFrom);
    periodStart.setHours(0, 0, 0, 0);
    const periodEnd = new Date(dateTo);
    periodEnd.setHours(23, 59, 59, 999);
...
async function getCustomerSaleRemainingQuantity(customerSaleId) {
    ...
    const complainedQuantity = sale.complaints.reduce((sum, c) => sum + c.affectedQuantity, 0);
...
async function createCustomerSaleComplaint(params) {
    const { customerSaleId, farmerId, complaintType, affectedQuantity, discountPercent, comment, } = params;
    ...
    if (affectedQuantity > remainingQuantity) {
        throw new Error(`Betroffene Menge (${affectedQuantity}) übersteigt Restmenge (${remainingQuantity})`);
    }
    // 2. Snapshots laden
    ...
    if (!farmer)
        throw new Error(`Farmer mit ID ${farmerId} nicht gefunden`);
    if (!product)
    ...
    try {
        ...
        if (diff !== 0) {
            ...
            const totalPriceSnapshot = pricePerUnitNum != null
                ...
                : null;
            await applyPackPlantStockChange(packPlantIdNum, productIdNum, diff, "INVENTORY", comment ?? "Inventur Packbetrieb", pricePerUnitNum, totalPriceSnapshot);
        }
...
app.post("/api/packplant-stock/sale", async (req, res) => {
    const { packPlantId, productId, customerId, units, date, pricePerUnit, comment, varietyId, farmerId, } = req.body;
    const packPlantIdNum = packPlantId ? Number(packPlantId) : 1;
    ...
    try {
        ...
        let varietyNameSnapshot = null;
        let farmerNameSnapshot = null;
        let finalVarietyId = null;
        let finalFarmerId = null;
        if (varietyId) {
        ...
        await applyPackPlantStockChange(packPlantIdNum, productIdNum, -unitsNum, "SALE", `Verkauf an ${customerNameSnapshot}`, finalPrice, totalPrice, sale.id // customerSaleId für Rückverfolgbarkeit
        );
...
app.post("/api/customer-sales/repair-snapshots", async (_req, res) => {
    try {
        ...
        let repairedCount = 0;
        for (const sale of sales) {
            const needsRepair = !sale.customerNameSnapshot ||
...
app.put("/api/product-prices/:id", async (req, res) => {
    ...
    try {
        ...
        for (const sale of affectedSales) {
            ...
            const priceAtSaleDate = await getPriceForCustomerProductAtDate(customerIdNum, productIdNum, sale.date);
...
app.post("/api/product-prices/backfill-valid-to", async (_req, res) => {
    try {
        ...
        const qualities = ["Q1", "Q2", "UEBERGROESSE"];
...
app.post("/api/customer-sale-complaints", async (req, res) => {
    const { customerSaleId, farmerId, complaintType, affectedQuantity, discountPercent, comment, } = req.body;
...
app.get("/api/customer-sale-complaints", async (req, res) => {
    ...
    try {
        const where = {};
        if (customerSaleId)
            where.customerSaleId = Number(customerSaleId);
        if (farmerId)
...
app.get("/api/packbetrieb/statistics", async (req, res) => {
    const { dateFrom, dateTo, productId, customerId, type // "SALE" | "COMPLAINT" | "INVENTORY" | "ALL"
     } = req.query;
    try {
        ...
        const salesWhere = {};
        if (dateFromParsed)
            salesWhere.date = { ...salesWhere.date, gte: dateFromParsed };
        if (dateToParsed)
            salesWhere.date = { ...salesWhere.date, lte: dateToParsed };
        if (productId)
            salesWhere.productId = Number(productId);
        if (customerId)
            salesWhere.customerId = Number(customerId);
        ...
        const complaintsWhere = {};
        if (dateFromParsed)
            complaintsWhere.createdAt = { ...complaintsWhere.createdAt, gte: dateFromParsed };
        if (dateToParsed)
            complaintsWhere.createdAt = { ...complaintsWhere.createdAt, lte: dateToParsed };
        if (productId) {
        ...
        const inventoryWhere = { reason: "INVENTORY" };
        if (dateFromParsed)
            inventoryWhere.createdAt = { ...inventoryWhere.createdAt, gte: dateFromParsed };
        if (dateToParsed)
            inventoryWhere.createdAt = { ...inventoryWhere.createdAt, lte: dateToParsed };
        if (productId)
            inventoryWhere.productId = Number(productId);
        ...
        if (dateFromParsed)
            packagingWhere.date = { ...packagingWhere.date, gte: dateFromParsed };
        if (dateToParsed)
            packagingWhere.date = { ...packagingWhere.date, lte: dateToParsed };
        if (productId)
            packagingWhere.productId = Number(productId);
        ...
        const wasteWhere = { reason: "SORTING_WASTE" };
        if (dateFromParsed)
            wasteWhere.createdAt = { ...wasteWhere.createdAt, gte: dateFromParsed };
        if (dateToParsed)
            wasteWhere.createdAt = { ...wasteWhere.createdAt, lte: dateToParsed };
        ...
        const inventoryZeroWhere = { reason: "INVENTORY_ZERO" };
        if (dateFromParsed)
            inventoryZeroWhere.createdAt = { ...inventoryZeroWhere.createdAt, gte: dateFromParsed };
        if (dateToParsed)
            inventoryZeroWhere.createdAt = { ...inventoryZeroWhere.createdAt, lte: dateToParsed };
...
app.put("/api/customer-sales/:id", async (req, res) => {
    ...
    try {
        ...
        if (oldMovement) {
            ...
            await applyPackPlantStockChange(1, existingSale.productId, Number(existingSale.quantityUnits), // Zurück ins Lager
            "SALE_CORRECTION", `Korrektur Verkauf #${id}`, Number(existingSale.unitPrice), Number(existingSale.totalAmount));
            // Neue Bewegung erstellen
            await applyPackPlantStockChange(1, newProductId, -newQuantityUnits, // Aus Lager
            "SALE", `Verkauf an ${customerNameSnapshot}`, Number(newUnitPrice), newTotalAmount);
...
async function applyPackPlantStockChange(packPlantId, productId, changeUnits, reason, comment, pricePerUnitSnapshot, totalPriceSnapshot, customerSaleId, packagingRunId) {
...
app.post("/api/packaging-runs", async (req, res) => {
    ...
    try {
        ...
        Number(productId), Number(quantityUnits), "PACKAGING_IN", `Verpackung Run ${run.id}`, undefined, // pricePerUnitSnapshot
...
app.put("/api/packaging-runs/:id", async (req, res) => {
    const { id } = req.params;
    const { date, productId, farmerId, varietyId, quantityUnits, reserved, wasteKg, rawInputKg, finishedKg, } = req.body;
    try {
        ...
        await applyPackStationStockChange(existingRun.packStationId, existingRun.farmerId, existingRun.varietyId, Number(existingRun.rawInputKg), "PACKAGING_OUT_CORRECTION", `Korrektur Verpackung Run #${id}`);
        // Alte Fertigware aus Packbetriebs-Lager raus
        await applyPackPlantStockChange(1, existingRun.productId, -Number(existingRun.quantityUnits), "PACKAGING_IN_CORRECTION", `Korrektur Verpackung Run #${id}`);
        ...
        await applyPackStationStockChange(existingRun.packStationId, newFarmerId, newVarietyId, -newRawInputKg, "PACKAGING_OUT", `Verpackung Run ${id}`);
        // Neue Fertigware in Packbetriebs-Lager rein
        // WICHTIG: Verwende das neue Produkt, falls es sich geändert hat
        await applyPackPlantStockChange(1, newProductId, newQuantityUnits, "PACKAGING_IN", `Verpackung Run ${id}`);
        res.json(updatedRun);
    }
    catch (err) {
...
app.put("/api/packstation/inventory-zero/:id", async (req, res) => {
    ...
    try {
        ...
        if (stockKg != null && stockKg !== undefined) {
            ...
            await applyPackStationStockChange(existingMovement.packStationId, existingMovement.farmerId, existingMovement.varietyId, originalStock, // Positive Änderung = Lagerstand zurück auf ursprünglichen Wert
            "INVENTORY_ZERO_CORRECTION", `Korrektur 'Auf 0'-Buchung #${id} - Rückgängigmachen`);
            ...
            await applyPackStationStockChange(existingMovement.packStationId, existingMovement.farmerId, existingMovement.varietyId, changeToNewStock, // Änderung von stockAfterCorrection auf newStockKg
            "INVENTORY_CORRECTION", newComment || `Lagerstand auf ${newStockKg} kg korrigiert`);
...
app.post("/api/delivery-plans/import-from-csv", async (_req, res) => {
    try {
        ...
        const projectRoot = process.cwd().endsWith("/src")
            ...
            : process.cwd();
        const csvPath = path_1.default.resolve(projectRoot, "planmengen.csv");
        ...
        if (farmerCount === 0) {
            return res.status(400).json({
                ...
                farmerIdsInCsv: Array.from(new Set(rawRows.map((r) => r.farmerId || r.farmerid))).filter(Boolean),
            });
        }
...
// ▶ Admin: Passwort für Farmer zurücksetzen
app.post("/api/admin/farmers/:farmerId/reset-password", async (req, res) => {
    const { farmerId } = req.params;
    const { newPassword } = req.body;
    if (!newPassword || newPassword.trim().length === 0) {
        return res.status(400).json({
    ...
    try {
        ...
        if (!farmer) {
            return res.status(404).json({
                error: "Farmer nicht gefunden",
            });
        }
        // Passwort hashen
        const passwordHash = await bcryptjs_1.default.hash(newPassword.trim(), 10);
        // Passwort im Farmer-Modell aktualisieren
        ...
        // Wenn Farmer einen User-Eintrag hat, auch dort aktualisieren
        if (farmer.email) {
            const user = await prisma.user.findUnique({
            ...
            if (user) {
                ...
                console.log(`Passwort für Farmer ${farmerId} (${farmer.email}) wurde auch im User-Eintrag aktualisiert`);
            }
        }
        console.log(`Passwort für Farmer ${farmerId} (${farmer.name}) wurde zurückgesetzt`);
        res.json({
            success: true,
            message: `Passwort für ${farmer.name} wurde erfolgreich zurückgesetzt`,
        });
    }
    catch (err) {
        console.error("Fehler in POST /api/admin/farmers/:farmerId/reset-password:", err);
        return res.status(500).json({
            error: "Fehler beim Zurücksetzen des Passworts",
            detail: String(err.message || err),
        });
...
app.post("/api/auth/login", async (req, res) => {
    ...
    try {
        ...
        if (user.password.startsWith("$2")) {
            ...
            passwordValid = await bcryptjs_1.default.compare(password, user.password);
        }
        else {
            // Klartext (alte User)
